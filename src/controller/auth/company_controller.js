...
async function verifyEmail(req, res, next) {
  try {
    ...
    if (!findEmployee && !findCompany) {
      res
        .status(200)
        .json({ statusCode: 403, success: false, message: "Email not exist" });
    } else {
...
async function verifyOtp(req, res, next) {
  try {
    ...
    res
      .status(200)
      .json({ statusCode: 200, success: true, message: "Otp is write" });
  } catch (e) {
    next(new ApiError(400, e.message));
  }
}
