...
async function createCompany(req, res, next) {
  try {
    ...
    if (companyValidation.error) {
      return next(
        new ApiError(403, companyValidation.error.details[0].message)
      );
    }
...
async function deleteCompany(req, res, next) {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "company delete successfully" });
  } catch (e) {
    next(new ApiError(400, e.message));
  }
}
...
async function viewCompanyOrProfile(req, res, next) {
  ...
  try {
    if (req.role === EMPLOYEE_ROLE) {
      ...
      const id = req.id;
      const company = await CompanyModel.findById({ _id: id }).populate(
        "workCategory"
      );
...
async function addEmployee(req, res, next) {
  try {
    ...
    if (findEmployee) {
      return next(
        new ApiError(400, "Already contain this email in your company")
      );
    }
    req.body.company = id;
...
async function getEmployee(req, res, next) {
  try {
    const emps = await EmployeeModel.find({ company: req.id })
      .populate("department")
      .populate("designation");
  ...
  try {
    ...
    if (role === EMPLOYEE_ROLE) {
      ...
      if (emp.email === email && comparePass === true) {
        ...
        const changePass = await EmployeeModel.findByIdAndUpdate(
          { _id: id },
      ...
      if (company.email === email && comparePass === true) {
        newPassword = hashPassword(newPassword);
        const changePass = await CompanyModel.findByIdAndUpdate(
          { _id: id },
          { $set: { password: newPassword } },
          { new: true }
        );
